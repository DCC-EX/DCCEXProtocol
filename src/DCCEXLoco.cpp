...
void Consist::removeLoco(Loco* loco) {
  for (ConsistLoco* conLoco=_first; conLoco; conLoco=conLoco->getNext()) {
    if (conLoco->getLoco()==loco) {
      // If source is entry, delete loco object
      // Delete loco pointer from list
    }
  }
}
