 ...
 */
    ...
    #define DCCEX_DEFAULT_MILLIS() millis()
  #elif (defined(__linux__) || defined(_WIN32) || defined(__APPLE__)) && !defined(ARDUINO)
    #include <chrono>
    static inline unsigned long dccex_host_millis() {
      const auto now = std::chrono::steady_clock::now();
      static const auto start = now;
      return (unsigned long)std::chrono::duration_cast<std::chrono::milliseconds>(now - start).count();
    }
    #define DCCEX_DEFAULT_MILLIS() dccex_host_millis()
  #endif
